...
import { getAllIndiaCoworkingSpaces } from "../src/libs/getAllIndiaCoworkingSpaces";
...
async function main() {
  console.log("--- COWORKING SPACES SCRAPER ---");
  if (testMode) {
    console.log("Mode: TEST (Bengaluru / Koramangala only)");
  } else if (cityFilter) {
    console.log(`Mode: SINGLE CITY — ${cityFilter}`);
  } else {
    console.log("Mode: ALL 37 CITIES");
  }
  console.log("");

  await prismaInstance.$connect();

  await getAllIndiaCoworkingSpaces({ testMode, cityFilter });
}
