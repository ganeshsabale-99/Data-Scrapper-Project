...
import { getAllIndiaTechParks } from "../src/libs/getAllIndiaTechParks";
...
async function main() {
  console.log("--- TECH PARKS SCRAPER ---");
  if (testMode) {
    console.log("Mode: TEST (Bengaluru only)");
  } else if (cityFilter) {
    console.log(`Mode: SINGLE CITY — ${cityFilter}`);
  } else {
    console.log("Mode: ALL 25 CITIES + NATIONAL");
  }
  console.log("");

  await prismaInstance.$connect();

  await getAllIndiaTechParks({ testMode, cityFilter });
}
