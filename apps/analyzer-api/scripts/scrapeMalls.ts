...
import { getAllIndiaMalls } from "../src/libs/getAllIndiaMalls";
...
async function main() {
  ...
  await getAllIndiaMalls({ testMode, cityFilter });
}
...
main()
  ...
  .catch((error) => {
    console.error("Mall scrape failed:", error instanceof Error ? error.message : String(error));
    process.exit(1);
  })
  .finally(async () => {
