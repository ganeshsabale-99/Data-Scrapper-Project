...
import { getAllIndiaStadiums } from "../src/libs/getAllIndiaStadiums";
...
async function main() {
  ...
  await getAllIndiaStadiums({ testMode, cityFilter });
}

main()
  .then(() => { console.log("\nScrape finished successfully."); process.exit(0); })
  .catch((error) => { console.error("Stadium scrape failed:", error instanceof Error ? error.message : String(error)); process.exit(1); })
