...
import { getAllIndiaHospitals } from "../src/libs/getAllIndiaHospitals";
...
async function main() {
  ...
  await getAllIndiaHospitals({ testMode, cityFilter });
}
...
main()
  ...
  .catch((error) => {
    console.error("Hospital scrape failed:", error instanceof Error ? error.message : String(error));
    process.exit(1);
  })
  .finally(async () => {
